      ...
      <div className="walletControl">
        ...
        {isOpen && (
          <div className="walletPopover" role="status">
            ...
            {error ? <p className="walletError"><AlertCircle size={14} />{error.message.toLowerCase().includes("provider") ? "No browser wallet was detected." : "Connection was not completed."}</p> : <p>Connect an injected wallet to use {botChain.name}.</p>}
      ...
      {isOpen && (
        <div className="walletPopover" role="dialog" aria-label="Wallet session">
          ...
          {isWrongNetwork ? (
            ...
            <div className="walletIdentity"><span className="walletIdentityIcon"><Check size={14} /></span><div><strong>{shortenAddress(address)}</strong><small>{balance ? `${Number(balance.formatted).toFixed(3)} ${balance.symbol}` : botChain.name}</small></div><button aria-label="Copy wallet address" title="Copy wallet address" type="button" onClick={copyAddress}><Copy size={14} /></button></div>
          )}
