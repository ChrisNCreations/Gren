...
const pendingPath = resolve(repoRoot, "contracts", "script", "deployments", "bot-chain-mainnet.pending.json");
const finalPath = resolve(repoRoot, "contracts", "script", "deployments", "bot-chain-mainnet.json");
...
async function main() {
  ...
  if (pending.network !== "bot-chain-mainnet" || pending.chainId !== 677) {
    throw new Error("Pending deployment metadata is not BOT Chain Mainnet configuration");
  }
  if (pending.policy?.bdexEnabled) {
    throw new Error("Mainnet artifact unexpectedly enables BDEX");
  }
